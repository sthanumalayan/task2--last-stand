...
let button=document.getElementById("start").firstChild;
...
function tracker(event){
    ...
    c.quadraticCurveTo(((135+x)/ 2)+move_tracker.x, 40+move_tracker.y, x+move_tracker.x, y+move_tracker.y);
    c.strokeStyle = 'blue';
    c.lineWidth = 2;
    c.stroke();
    //end point
    c.beginPath();
    c.arc(x+move_tracker.x, y+move_tracker.y, 5, 0, Math.PI * 2);
    c.fillStyle = 'black';
    c.fill();
};
function delay(ms) {
...
function call_shoot(){
    shoot(135+move_tracker.x, 50 +move_tracker.y, ((135+x)/2) +move_tracker.x, 40+move_tracker.y, x +move_tracker.x, y+move_tracker.y);
}
function pause(){
    mover.removeEventListener("mousemove",tracker);
    mover.removeEventListener("click",call_shoot);

...
function hit(z,p,q){
    ...
    if(z.position=="left"){
        if((10+(z.x)/4)-40<=p && (10+(z.x)/4)+40>=p && flag){
            return true;
        }
    }
    else if(z.position=="right"){
        if(((140+(z.x-850)/4)-40<=p) && (140+(z.x-850)/4)+40>=p &&flag ){
            return true;
        }
    }
    return false;
...
function spawn_zombies(){
    if(timer.innerText!='0' &&!over &&!pause_flag){
        ...
        z2=new Zombie(document.getElementById("canvas").width-200,100,"right");
        zombies.push(z1);
        zombies.push(z2);
        move_zombie(z1);
        move_zombie(z2);
...
async function move_zombie(z){
        while(!z.reached && !pause_flag ){
            ...
            await delay(500);
            if(pause_flag)break;
            if(z.position=="left" && ((10+(z.x)/4)-40<=pl.x+120 && ((10+(z.x)/4))+40>=pl.x+120)){
            ...
            if(z.position=="right"&&((140+((z.x-1200)/4))-40<=pl.x+120 && (140+((z.x-1200)/4))+40>=pl.x+120)){
                function check(zombie){
                    return zombie!=z;
                }
                zombies=zombies.filter(check);
